...
public class Part2 {
    ...
    public static void main(String[] args) { new Part2().start(); }  // We get out of static context to use additional classes more easy
    public void start() {
        ...
        Tile santa = new Tile(0, 0);
        Tile roboSanta = new Tile(0, 0);

        for (int i=0; i < input.length(); i++) {
            if (i % 2 == 0) { // not the most beautifulest version but it works
                switch (input.charAt(i)) {
                    case '^': santa.x++; break;
                    case '>': santa.y++; break;
                    case 'v': santa.x--; break;
                    case '<': santa.y--; break;
                }

                if (!tiles.contains(santa.toString()))
                    tiles.add(santa.toString());

            } else {
                switch (input.charAt(i)) {
                    case '^': roboSanta.x++; break;
                    case '>': roboSanta.y++; break;
                    case 'v': roboSanta.x--; break;
                    case '<': roboSanta.y--; break;
                }

                if (!tiles.contains(roboSanta.toString()))
                    tiles.add(roboSanta.toString());
            }
